...
Chainlink scores the liquidation challenge by running price scenarios against the live contract during the 24 hours after the submission deadline, **2026-09-13 16:00 UTC**. Both workflows have been deployed to the private registry since 2026-09-12 06:19 UTC (`liquidation-protection-production` `00cdbaa2…`, `bond-monitor-production` `0045bd36…`; record in `docs/cre-evidence/deployed-20260912.txt`), so the defender runs on the CRE network every 30 s. `scripts/defend-loop.sh` is kept as the backup if the deployed workflow stops executing before or during the window:
...
Disclosure: this is a fallback, not the design. The simulator is not a TEE, so nothing about these runs is attested, and the secrets, wallet key and policy alike, are read from `workflow/.env` on our machine instead of being released by the Vault DON into an enclave. It is started only if `cre execution list liquidation-protection-production` stops showing fresh `SUCCESS` rows. Keep the Sepolia wallet funded: each intervention is up to two transactions at a 150,000 gas limit.
...
- `cre workflow simulate` needs `cre login` and `CRE_ETH_PRIVATE_KEY` in `.env` even for the Hedera workflow; the simulator is not a real TEE (it says so in its banner, right under the constraint it resolved, `AWS Nitro in us-west-2`); whether the deployed run honoured that constraint is not visible from the CLI (`docs/cre-evidence/challenge.md`).
- **Production logs are visible after all, once per node.** The simulator banner says user logs "will not leave the TEE", but `cre execution logs <id>` on the deployed workflow returns the handler's log line from every DON node. Treat anything a confidential handler logs as public: ours logs a coverage bucket and a plan word, never a threshold. The deployed bond-monitor still delivers its own verdict (`deliver: "direct"` in `config.production.json`) rather than depending on a log-line relay.
