...
import { decideBond, decideLiq, hfX100, shouldDeliver } from "./decide"

const policy = { warnBelowBps: 13000n, freezeBelowBps: 11000n, defaultBelowBps: 5000n }
const liq = { triggerHf: 115n, targetHf: 130n, maxRepay: 300000n, maxDeposit: 200n, cooldownSecs: 90n }
...
describe("decideLiq", () => {
  ...
  test("repays just enough to reach target", () => {
    expect(decideLiq(ref, liq)).toEqual({ hf: 111n, repay: 100000n, deposit: 0n, reason: "defend" })
  })
  test("caps repay then tops up collateral so hf >= target", () => {
    ...
    expect(hfX100(ref.collateral + p.deposit, ref.debt - p.repay, ref.price)).toBeGreaterThanOrEqual(130n)
  })
